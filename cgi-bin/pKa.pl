...
if ($flow eq 'calibrate') {
	print <<EOF;
	<h2>Calibration</h2>
	<form action="/~pfleming/cgi-bin/pKa.pl" method="post">
	<select name="time">
...
EOF
	...
	<p><form action="/~pfleming/cgi-bin/pKa.pl" method="post">
...
EOF
	...
	for ($i=0; $i <= $tit_time; $i += 1) {
		...
		if ($base_mmol == 0) {
			...
			my $H_conc = sqrt($acid_conc * $Ka);
			$pH[$i] = sprintf("%.2f", -log10($H_conc));
			...
			# my $a = 1;
			# my $b = $base_mmol + $Ka * $vol_tot[$i];
			# my $c = -$Ka * $vol_tot[$i] * ($acid_mmol - $base_mmol);
